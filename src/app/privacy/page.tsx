...
export default function PrivacyPage() {
  ...
  const bangla = useMemo(
    () => ls?.privacyContentBn ?? defaultBangla,
    [ls]
  )
  ...
  const wrapper = useMemo(() => {
    ...
    if (bgStyle === 'solid') {
      return { className: 'min-h-screen', style: { background: solidColor, color: solidContrast } }
    }
  ...
  return (
    <AuthSessionProvider>
      <div className={wrapper.className} style={wrapper.style}>
        ...
        <Header showNavigation={false} showUserActions={true} onMobileMenuToggle={() => setIsMobileSidebarOpen(true)} />
          ...
          <main className="flex-1 overflow-auto p-6 pt-24 pb-20">
            <div className="max-w-6xl mx-auto space-y-8">
              ...
              <div className="relative overflow-hidden rounded-2xl border border-white/30 dark:border-white/20 bg-white/20 dark:bg-white/10 p-8 md:p-12">
                <h1 className="text-4xl md:text-5xl font-bold text-gray-900 dark:text-gray-100">Privacy Policy</h1>
                ...
                <aside className="md:col-span-3">
                  <div className="rounded-xl border border-white/30 dark:border-white/20 bg-white/20 dark:bg-white/10 p-4">
                    <nav className="space-y-1 text-sm">
                      ...
                      <a href="/cookies" className="block rounded-lg px-3 py-2 text-gray-700 dark:text-gray-300 hover:bg-white/20">
                        Cookies Policy
                      </a>
                  ...
                  <div className="bg-white/20 dark:bg-white/10 backdrop-blur-md border border-white/30 dark:border-white/20 rounded-xl p-6 md:p-8 shadow-lg">
                    {!mounted ? (
                      ...
                      (() => {
                        ...
                        const isTitle = first.length > 0 && (tab === 'en' ? /privacy policy/i.test(first) : /গোপনীয়তা|Privacy Policy/.test(first))
                        const body = isTitle ? parts.slice(1).join('\n') : content
                        const title = isTitle ? first : (tab === 'en' ? 'Privacy Policy' : 'গোপনীয়তা নীতি (Privacy Policy)')
                        return (
                          <>
